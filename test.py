# Analyze the performance of the test set
import numpy as np
import torch
...
# It can test some data points (such as fixed time)
# and can also test the whole flow field data points
def test_part_data(x, y, u, v, p):
    ...
    u_predict, v_predict, p_predict, f_equation_x, f_equation_y = f_equation_inverse(x_test, y_test, pinn_net)
...
v_test = v[index_test, :]
p_test = p[index_test, :]
mse_all, mse_prediction, mse_equation, mse_u, mse_v, mse_p = test_part_data(x_test, y_test, u_test, v_test, p_test)
